...
def create_check_prro(checktype, fiscal_number, edrpo=2836413030, id_prro=1, date_check=0, last_hash_check=0,
                      check_xml='') -> bytes:
    ...
    if checktype == '2':  # Z-report
        ...
        for i in check_xml:
            ...
            if name != '':
                check += f'<M NM="{name}" SMI="{string_xml[1]}" SMO="0" T="{string_xml[2]}"/>'
        check += f'<NC NI="0" NO="0"/></Z>'
    elif checktype == '3':  # 0-check
        check += f'<C T="108"></C>'
    elif checktype == '1':  # check_order
        check_xml = check_xml.split('&\?')
        ...
        check += f'<M N="{count+1}" T="{nal_beznal}" NM="{nal_beznal_txt}" SM="{summa}" M="0" RM="0"/>'
...
class TerminalCom:
    ...
    def checkdfs(self, data_time, check_xml, localnumber, checktype, last_hash_check):
        self.res_id = 0
        self.res_status = ''
        self.discription_error = ''
        self.error = True
        strtime = data_time  # .encode('utf-8')
        logging_report(f'str_time {strtime}')
        check_xml = create_check_prro(checktype, self.fnrro, date_check=strtime, last_hash_check=last_hash_check, check_xml=check_xml)
        ...
        while _count > 0:
            try:
                res = stub.sendChkV2(vbk.Check(rro_fn=str(self.fnrro), date_time=int(strtime),
                                               check_sign=check_sign, local_number=int(localnumber),
